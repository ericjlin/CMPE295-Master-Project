...
const generateMarkers = () =>{
  ...
  const cities =
[{name:"San Francisco",coordinates:{latitude: 37.773972,
  longitude:  -122.431297 }}, {name:"San Jose",coordinates:{ latitude: 37.335480,
    longitude: -121.893028 }}]
  // const cities =
// [{name:"San Francisco",coordinates:{{ latitude: 37.78825,
//   longitude: -122.4324 }} , "Daly City", "San Mateo", "Palo Alto",
...
export default function TabTwoScreen() {
  ...
  const [cities, setCities] = useState([{name:"San Francisco",coordinates:{latitude: 37.773972,
    longitude:  -122.431297 }}, {name:"San Jose",coordinates:{ latitude: 37.335480,
      longitude: -121.893028 }}])
  ...
  cities.forEach((city)=>{
    console.log(city)
    axios.get(`http://127.0.0.1:3000/user/getCityAve?city=${city}`).then((response)=>{
     console.log(response.data)  
     const message =  response.data.message
     if (message.length > 0)
         setCityData(city, `Average ${message[0].type.toUpperCase()}: ${message[0].average.toFixed(2)} `)
